...
async function markField(id, field, done, dateKey, isRevision) {
  const now = new Date();
  ...
  if (done) {
    await activityLogRepo.record(dateKey, "dsa", 1, `${isRevision ? "revision-" : ""}${field}:${id}`);
  }
  return updated;
}

export const markTheory = (id, done, dateKey = todayKey(), isRevision = false) =>
  markField(id, "theory", done, dateKey, isRevision);
export const markPractice = (id, done, dateKey = todayKey(), isRevision = false) =>
  markField(id, "practice", done, dateKey, isRevision);
